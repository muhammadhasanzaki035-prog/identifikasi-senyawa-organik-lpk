...
if pilihan_halaman == "🏠 HALAMAN UTAMA":
    ...
    # Penambahan karakter edukasi pendamping di Petunjuk Penggunaan
    col_char, col_text = st.columns([1, 5])
    with col_char:
        st.markdown("<h1 style='text-align: center; font-size: 4rem; margin: 0;'>👨‍🔬</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; font-weight: bold; color: #0f766e;'>Prof. Organic</p>", unsafe_allow_html=True)
    with col_text:
        st.markdown("""
        ### **Petunjuk Penggunaan dari Prof. Organic**
        > "Halo! Saya akan mendampingi perjalanan belajarmu di lab virtual ini. Ikuti langkah praktis berikut ya!"
        
        1. **Pelajari Materi Teori:** Klik menu **BAB I hingga BAB IV** pada bilah navigasi kiri untuk mengakses catatan laboratorium komprehensif, sifat fisika-kimia, serta pemodelan persamaan reaksi kualitatif.
        2. **Uji Pemahaman di Ruang Simulasi:** Buka menu **🔬 POST TEST** untuk melakukan *blind test* terhadap sampel misterius. Analisis perubahan warna atau endapan langsung di tabung reaksi visual!
        """)
    ...
    * **Alkohol ($R-OH$):** Memiliki gugus hidroksil. Suku rendah larut dalam air karena sanggup membentuk ikatan hidrogen. Dibedakan menjadi alkohol primer, sekunder, dan tersier berdasarkan posisi gugus hidroksil pada atom karbon.
    * **Eter ($R-O-R'$):** Isomer fungsi alkohol yang tidak memiliki ikatan hidrogen antarmolekulnya sehingga titik didihnya jauh lebih rendah daripada alkohol dengan massa rumus serupa.
    * **Fenol ($C_6H_5OH$):** Gugus $-OH$ terikat langsung pada cincin benzena. Bersifat asam lemah karena ion fenoksidanya distabilkan secara efektif oleh resonansi awan elektron cincin aromatik.

    #### **B. Reaksi Kimia Utama & Analisis Diferensiasi**

    ##### **1. Pereaksi Lucas ($HCl + ZnCl_2$)**
    Digunakan untuk mendiferensiasi jenis alkohol berdasarkan kecepatan pembentukan alkil klorida hidrofobik yang mengeruhkan larutan pada suhu kamar:
    * Alkohol Tersier: Membentuk emulsi keruh seketika (instan).
    * Alkohol Sekunder: Membentuk emulsi setelah didiamkan selama 5-10 menit (cepat bereaksi dengan pemanasan).
    * Alkohol Primer: Tidak bereaksi atau tetap bening pada suhu kamar.
    """)
    st.latex(r"R_3C-OH + HCl \xrightarrow{ZnCl_2} R_3C-Cl \downarrow + H_2O")
    
    st.markdown("""
    ##### **2. Pereaksi Jones (Oksidasi Alkohol)**
    Kromium trioksida dalam asam sulfat mengubah warna pereaksi dari jingga menjadi hijau akibat reduksi Kromium(VI) menjadi Kromium(III). Alkohol primer dioksidasi menjadi asam karboksilat, alkohol sekunder dioksidasi menjadi keton, sedangkan alkohol tersier negatif (tidak teroksidasi).
    """)
    ...
    st.markdown("""
    ##### **3. Uji Iodoform**
    Spesifik untuk mendeteksi komponen metil karbinol ($R-CH(OH)-CH_3$) seperti pada 2-butanol, ditandai dengan terbentuknya endapan kristal kuning iodoform ($CHI_3$) beraroma khas.
    """)
    ...
    st.markdown("""
    Aldehida ($R-CHO$) dan keton ($R-CO-R'$) merupakan isomer fungsional yang sama-sama mengandalkan aktivitas gugus fungsi karbonil ($C=O$).

    #### **A. Reaksi Adisi Nukleofilik Karbonil**
    * **Adisi Natrium Bisulfit ($NaHSO_3$):** Karbonil aldehida atau metil keton suku rendah bereaksi positif menghasilkan produk adisi padat berupa kristal putih yang terendapkan.
    """)
    st.latex(r"R-CHO + NaHSO_3 \rightarrow R-CH(OH)-SO_3Na \downarrow")
    
    st.markdown("""
    #### **B. Uji Daya Reduksi Aldehida (Diferensiasi Komponen)**
    Aldehida adalah agen reduktor kuat karena mengikat atom hidrogen langsung pada karbon karbonil, sedangkan keton bernilai negatif (tidak reaktif).

    ##### **1. Uji Tollens (Cermin Perak)**
    Mereduksi ion kompleks perak beramoniak $[Ag(NH_3)_2]^+$ menjadi endapan logam perak murni yang menempel mengkilap di dinding tabung reaksi.
    """)
    st.latex(r"R-CHO + 2\ [Ag(NH_3)_2]^+ + 3\ OH^- \rightarrow R-COO^- + 2\ Ag \downarrow + 4\ NH_3 + 2\ H_2O")
    
    st.markdown("""
    ##### **2. Uji Fehling (Reduksi Kupri)**
    Mereduksi ion Tembaga(II) kompleks tartrat (Fehling A + B) menjadi endapan merah bata Tembaga(I) oksida ($Cu_2O$).
    """)
    ...
    st.markdown("""
    Asam karboksilat mengandung gugus fungsi karboksil ($-COOH$), kombinasi dari karbonil dan hidroksil yang menghasilkan keasaman tinggi dibandingkan alkohol.

    #### **A. Reaksi Kimia Utama**
    
    * **Reaksi dengan Basa Lemah ($NaHCO_3$):** Sifat asamnya mendonasikan proton untuk mengurai bikarbonat secara instan menjadi gas $CO_2$. Jika gas dialirkan ke air barit, akan terbentuk endapan barium karbonat yang keruh.
    """)
    st.latex(r"R-COOH + NaHCO_3 \rightarrow R-COONa + H_2O + CO_2 \uparrow")
    st.latex(r"CO_2 + Ba(OH)_2 \rightarrow BaCO_3 \downarrow + H_2O")
    
    st.markdown("""
    * **Esterifikasi Fischer:** Kondensasi langsung antara asam karboksilat dan alkohol dibantu katalis asam sulfat ($H_2SO_4$) pekat panas untuk memproduksi senyawa ester yang beraroma harum buah-buahan.
    """)
    st.latex(r"R-COOH + R'-OH \xrightarrow{H_2SO_4,\ \Delta} R-COOR' + H_2O")
    
    st.markdown("""
    #### **B. Uji Asam Hidroksamat (Identifikasi Golongan Ester)**
    Senyawa derivat ester tidak bereaksi dengan asam-basa biasa melainkan dikondensasikan dengan hidroksilamin ($NH_2OH$) menghasilkan asam hidroksamat. Senyawa ini berkoordinasi kuat dengan ion $Fe^{3+}$ membentuk senyawa kompleks kelat berwarna violet intens.
    """)
